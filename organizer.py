...
2. TAXONOMY (Line ~50)
   - PREFERRED_TAXONOMY: 双轨分类法定义

3. PROFILE & CACHE MANAGEMENT (Line ~125)
   - load_user_profile(): 加载用户画像
   - load_cache() / save_cache(): 缓存读写
   - load_analysis_results() / save_analysis_results(): 分析结果存储
   - get_analysis_results_file(): 生成分析结果文件名
   - refresh_cache_from_zotero(): 从Zotero刷新缓存

4. COLLECTION MANAGEMENT (Line ~309)
   - find_collection_by_path(): 按路径查找集合
   - ensure_collection_path(): 确保集合路径存在

5. AI CLASSIFICATION (Line ~487)
   - parse_note_content(): 解析笔记内容
   - extract_keywords_from_note(): 提取关键词
   - format_taxonomy_for_prompt(): 格式化分类法供AI使用
   - ai_dual_classify_batch(): AI批量分类
   - generate_report_filename(): 生成报告文件名
   - generate_global_analysis_report(): 生成全局分析报告

6. ITEM FETCHING (Line ~1166)
   - get_all_subcollection_keys(): 获取所有子集合
   - fetch_all_items(): 获取所有条目

7. ITEM PROCESSING (Line ~1299)
   - validate_collection_keys(): 验证集合键值
   - add_item_to_collections(): 添加条目到集合

8. MAIN PROCESSING (Line ~1452)
   - main(): 主程序入口
================================================================================
"""
